...
public class RCIMClientModule extends ReactContextBaseJavaModule {
  ...
  @ReactMethod
  public void resendMessageById(final Integer messageId, final String pushContent, final String pushData, final String eventId) {
    try {
      RongIMClient.getInstance().getMessage(messageId, new ResultCallback<Message>() {
        ...
        public void onSuccess(Message message) {
          ...
          if (message == null) {
            onSendMessageError(eventId, null, ErrorCode.PARAMETER_ERROR, "messageId invalid");
            return;
          }

          if (message.getSentStatus() != SentStatus.FAILED) {
            onSendMessageError(eventId, null, ErrorCode.PARAMETER_ERROR, "SentStatus invalid");
          }
  ...
  private ISendMediaMessageCallback createSendMessageCallback(final String eventId) {
    return new ISendMediaMessageCallback() {
      ...
      public void onAttached(Message message) {
        if (message != null) {
          WritableMap map = createEventMap(eventId, "attached");
          map.putMap("message", toJSON(message));
  ...
  @ReactMethod
  public void deleteRemoteMessages(int type, String targetId, ReadableArray messages, final Promise promise) {
  ...
  @ReactMethod
  public void getMessageCount(int conversationType, String targetId, final Promise promise) {
    RongIMClient.getInstance().getMessageCount(ConversationType.setValue(conversationType), targetId, new RongIMClient.ResultCallback<Integer>() {
      @Override
