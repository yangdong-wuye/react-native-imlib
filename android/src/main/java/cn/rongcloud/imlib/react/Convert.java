...
class Convert {
    ...
    static WritableMap toJSON(Uri message) {
        WritableMap map = Arguments.createMap();
        map.putString("notificationId", message.getQueryParameter("notificationId"));
        map.putString("pushType", message.getQueryParameter("pushType"));
        map.putString("pushId", message.getQueryParameter("pushId"));
        map.putString("pushTitle", message.getQueryParameter("pushTitle"));
        map.putString("pushFlag", message.getQueryParameter("pushFlag"));
        map.putString("pushContent", message.getQueryParameter("pushContent"));
        map.putString("pushData", message.getQueryParameter("pushData"));
        map.putString("objectName", message.getQueryParameter("objectName"));
        map.putString("senderId", message.getQueryParameter("senderId"));
        map.putString("senderName", message.getQueryParameter("senderName"));
        map.putString("senderPortraitUrl", message.getQueryParameter("senderPortraitUrl"));
        map.putString("targetId", message.getQueryParameter("targetId"));
        map.putString("targetUserName", message.getQueryParameter("targetUserName"));
        map.putInt("conversationType", Integer.parseInt(message.getQueryParameter("conversationType")));
        map.putString("extra", message.getQueryParameter("extra"));
        return map;
    }
    ...
    static WritableMap toJSON(String objectName, MessageContent content) {
        ...
        if (content.getUserInfo() != null && content.getUserInfo().getUserId().length() > 0) {
          map.putMap("userInfo",Convert.toJson(content.getUserInfo()));
        }
  ...
  static WritableMap toJson(UserInfo userInfo) {
        ...
        map.putString("portraitUrl",userInfo.getPortraitUri().toString());
        map.putString("name",userInfo.getName());
        map.putString("extra",userInfo.getExtra());
        return map;
    ...
    static Message toMessage(ReadableMap map) {
        ...
        Message message = Message.obtain(map.getString("targetId"), conversationType, content);
        if (map.hasKey("objectName")) {
            message.setObjectName(map.getString("objectName"));
        }
        if (map.hasKey("messageId")) {
            message.setMessageId(map.getInt("messageId"));
        }
        if (map.hasKey("messageUId")) {
            message.setUId(map.getString("messageUId"));
        }
        if (map.hasKey("messageDirection")) {
            message.setMessageDirection(Message.MessageDirection.setValue(map.getInt("messageDirection")));
        }
        if (map.hasKey("senderUserId")) {
            message.setSenderUserId(map.getString("senderUserId"));
        }
        if (map.hasKey("sentTime")) {
            message.setSentTime((long)map.getDouble("sentTime"));
        }
        if (map.hasKey("receivedTime")) {
            message.setReceivedTime((long)map.getDouble("receivedTime"));
        }
        if (map.hasKey("sentStatus")) {
            message.setSentStatus(Message.SentStatus.setValue(map.getInt("sentStatus")));
        }
        if (map.hasKey("receivedStatus")) {
            message.setReceivedStatus(new Message.ReceivedStatus(map.getInt("receivedStatus")));
        }
        if (map.hasKey("extra")) {
            message.setExtra(map.getString("extra"));
        }
